...
const HoverEffect = ({ items, className, isVoted }) => {
  const [hoveredIndex, setHoveredIndex] = useState(null);

  return (
    <div
      className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-10 ${className} cursor-pointer`}
    >
      {items.map((item, idx) => (
        <Link
          to={isVoted ? "" : `/vote/nominees/${item.id}`}
